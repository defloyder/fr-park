...
const roadDividerMarkingWidth = ['interpolate', ['linear'], ['zoom'], 16.8, 1.3, 18.5, 2.4, 20, 3.4];
const roadCenterMarkingWidth = ['interpolate', ['linear'], ['zoom'], 16.8, 1.6, 18.5, 3, 20, 4.2];
    ...
    for (let laneCount = 2; laneCount <= 8; laneCount += 1) {
        for (let dividerIndex = 1; dividerIndex < laneCount; dividerIndex += 1) {
            layers.push(roadLineLayer({
                ...
                opacity: ['interpolate', ['linear'], ['zoom'], 17.2, 0.62, 18.5, 0.84, 20, 0.92],
                dasharray: [4.8, 3],
...
function createTwoWayLaneDividerLayers({ source, sourceLayer, filter }) {
    return [
        roadLineLayer({
            ...
            opacity: ['interpolate', ['linear'], ['zoom'], 17, 0.68, 18.5, 0.9, 20, 0.96],
...
function createRoadEdgeMarkingLayers({ source, sourceLayer, idPrefix, filter, negativeOffset, positiveOffset }) {
    return [
        roadLineLayer({
            ...
            opacity: ['interpolate', ['linear'], ['zoom'], 16.8, 0.52, 18.5, 0.78, 20, 0.9],
