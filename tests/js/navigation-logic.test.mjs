...
test('map settings expose selectable GPS cursor icons', () => {
    ...
    assert.match(mapSource, /USER_LOCATION_MODEL_SOURCE_ID/);
    assert.match(mapSource, /USER_LOCATION_MODEL_EXTRUSION_LAYERS/);
    ...
    assert.match(mapSource, /USER_LOCATION_MODEL_VERTICAL_SCALE = 1/);
    ...
    assert.match(mapSource, /'auralith-nav-cyan': `\$\{GPS_CURSOR_RUNTIME_MODEL_ASSET_BASE\}nissan_gt-r_2008\.glb`/);
    assert.match(mapSource, /'auralith-nav-graphite': `\$\{GPS_CURSOR_RUNTIME_MODEL_ASSET_BASE\}nissan_fairlady_z_s30240z_1978\.glb`/);
